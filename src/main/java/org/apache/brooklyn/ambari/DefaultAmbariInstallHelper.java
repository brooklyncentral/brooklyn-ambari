...
public class DefaultAmbariInstallHelper implements AmbariInstallHelper {
    ...
    private String getAptRepo() {
        return ifExecutableElse1("apt-get", chainGroup(commandToDownloadUrlAs(UBUNTU_AMBARI_REPO_LOCATION, UBUNTU_REPO_LIST_LOCATION),
    ...
    private String setupCentos6Repo(SshMachineLocation sshMachineLocation) {
        ...
        if (osDetailsVersion.startsWith("6")) {
            return ifExecutableElse1("yum", commandToDownloadUrlAs(CENTOS_6_AMBARI_REPO_LOCATION, CENTOS_REPO_LIST_LOCATION));
        } else {
            return ifExecutableElse1("yum", commandToDownloadUrlAs(CENTOS_5_AMBARI_REPO_LOCATION, CENTOS_REPO_LIST_LOCATION));
        }
    }

    private String setupSuseRepo() {
        return ifExecutableElse1("zypper", commandToDownloadUrlAs(SUSE_AMBARI_REPO_LOCATION, SUSE_REPO_LIST_LOCATION));
    }
